...
var formatTime = function (seconds) {
    ...
    var remainHours = Math.floor(seconds / 3600);
    var remainMinutes = Math.floor((seconds % 3600) / 60);
    var remainSeconds = seconds % 60;
    var str = sgn + (remainHours < 10 ? '0' : '') + remainHours + ':';
...
var parseTime = function (str) {
    var spl = str.split(":");
    var remainHours = parseInt(spl[0].charAt(0) == "0" ? spl[0].charAt(1) : spl[0]);
...
var GLaDOSEnhancerPlusInit = function () {
    ...
    var processAjaxUpdate = function (data) {
        // Wrap whole document
        var wrapped = jQuery(data);
        
        // Update progress bar
        var ajOverallProgressBar = wrapped.find("#overall_progress_bar");
        overallProgressBar.attr("style", ajOverallProgressBar.attr("style"));
        ...
        jQuery("#game_rows").children(".game_row").each(function (index, elt) {
            elt = jQuery(elt);
            var newElt = wrapped.find("#" + elt.attr("id"));
            var gameProgressBar = elt.children(".game_progress");
            gameProgressBar.attr("style", newElt.children(".game_progress").attr("style"));
            gameProgressBar.attr("class", newElt.children(".game_progress").attr("class"));
            elt.children(".game_cpus").html(newElt.children(".game_cpus").html());
        });
...
// Run init after jQuery is ready

GLaDOSEnhancerPlusInit();
